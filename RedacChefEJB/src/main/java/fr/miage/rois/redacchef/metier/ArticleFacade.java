...
public class ArticleFacade extends AbstractFacade<Article> implements ArticleFacadeLocal {
    ...
    @PersistenceContext(unitName = "fr.miage.rois_RedacChefEJB_ejb_1.0-SNAPSHOTPU")
    ...
    public List<Article> listeArticlesByTitre(Titre titre) {
        return getEntityManager()
                .createQuery("SELECT a FROM Article a WHERE a.idtitre = :titre AND a.valide = false")
                .setParameter("titre", titre.getIdtitre())
                .getResultList();
    }
    ...
    public void valideArticle(Article article) {
        getEntityManager()
                .createQuery("UPDATE Article a SET valide = true WHERE a.idarticle = :article")
    ...
    public List<Article> listeArticlesValide() {
        return getEntityManager()
                .createQuery("SELECT a FROM Article a WHERE a.valide = true")
                .getResultList();
    }
